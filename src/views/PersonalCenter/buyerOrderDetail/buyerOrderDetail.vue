<template>
  <div>
      ...
      <div class="content">
        <div class="step clear">
          <div class="left">
            ...
            <p class="icon" v-if="item.orderVo.order_status==4">
              ...
              <span
                style="display:block;"
                v-if="item.orderVo.orderStatesDesc"
              >{{item.orderVo.orderStatesDesc}}</span>
            </p>
          ...
          <div v-if="OrderProcessList.length" class="center" style="padding:15px;flex:1;">
            ...
            <div style="height:80px;overflow-y:auto;" class="steps">
              <el-steps direction="vertical">
                <el-step
                  :title="TimeForma2(item.createtime)"
                  icon="el-icon-s-tools"
                  v-for="item in OrderProcessList"
                  :key="item.id"
                  :description="item.ordesc"
                ></el-step>
              </el-steps>
            </div>
          </div>
          ...
          <ul>
            ...
            <li class="item">
              ...
              <div style="display:flex;">
                <div style="margin-right:10px;" v-if="item.orderVo.billType">
                  <el-button size="mini">
                    <a
                ...
                <div style="margin-right:10px;">
                  <el-button size="mini" v-if="item.orderVo.customerContractUrl">
                    <a
                      :href="`${baseURL}api-order/customerCenter/downLoad?urls=${item.orderVo.customerContractUrl}&access_token=${access_token}`"
                    >下载合同(已上传)</a>
                  </el-button>
                </div>
                <el-upload
                ...
                >
                  <el-button size="mini">上传合同</el-button>
                </el-upload>
              </div>
            </li>
            ...
            <li class="item">
              ...
              <div class="center">
                <div class style="border-bottom:1px solid #ddd;padding-bottom:10px">
                  ...
                  >总金额：￥{{item.orderVo.order_amount | toFixed(2)}}</p>
                  <p class style="margin-right:10px;">已付金额：￥{{item.orderVo.alreadyPay | toFixed(2)}}</p>
                  <p
                    class="green"
                    style="margin-right:10px;"
                  >未付金额：￥{{item.orderVo.shouldPay | toFixed(2)}}</p>
                ...
                <div
                  v-if="item.orderVo.need_pre_pay && !item.orderVo.prePayButton && item.orderVo.prePayChannel"
                  :class="item.orderVo.order_status==0?'color':'green'"
                  class="paychannel"
                >
                  <span>{{item.orderVo.prePayTime | formatDate}}</span>
                  <span>{{item.orderVo.prePayChannel | prePayChannel}}</span>
                ...
                <div
                  v-else-if="item.orderVo.prePayChannel"
                  :class="item.orderVo.order_status==0?'color':'green'"
                  class="paychannel"
                >
                  <span>{{item.orderVo.prePayTime | formatDate}}</span>
        ...
        <div class="list clear">
          <table border="1">
            ...
            <tbody>
              <template v-for="item0 in orderInfoList">
                <div :key="item0.id"></div>
                <template v-for="(value,index) in item0.list">
                    ...
                    <td colspan="8">
                      <div style="cursor:pointer;display:flex;" @click="chipShop(value)">
                        ...
                        <div>
                          ...
                          <span
                            class="tag bgBlu"
                            v-if="value.tag==2 && value.isAgent"
                          >{{value.tag | tagFilter}}</span>
                        </div>
                      </div>
                    </td>
                  </tr>
                  <tr :key="value.id">
                    ...
                    <td>
                      <div
                        v-if="value.good_price"
                      >{{value.priceunit?'$':'￥'}}{{value.good_price | toFixed(value.priceunit?3:2)}}</div>
                    </td>
                      ...
                      <el-popover placement="bottom" trigger="hover">
                        <div v-if="!value.goods_type" style="white-space:nowrap;">
                          ...
                          <p v-if="value.pay_channel">
                            ...
                            <span
                              v-if="value.pay_channel && value.pay_channel!=3 && value.prePayNo"
                              class="blu"
                              style="margin-right:10px;"
                            >{{value.prePayNo}}</span>
                          ...
                          <p v-if="value.final_pay">
                            <span
                              style="margin-right:10px;"
                            >尾 &nbsp;&nbsp;款：￥{{value.final_pay | toFixed(2)}}</span>
                            <span v-if="value.last_pay_channel==3" style="margin-right:10px;">
                              <ImgE :src="value.finalPayNo" :W="40" :H="40" :isBig="true"></ImgE>
                            </span>
                            <span
                              v-if="value.last_pay_channel && value.last_pay_channel!=3 && value.finalPayNo"
                              class="blu"
                              style="margin-right:10px;"
                            >{{value.finalPayNo}}</span>
                            <span
                              v-if="value.last_pay_channel"
                              style="margin-right:10px;"
                            >（{{value.last_pay_channel | prePayChannel}}）</span>
                        ...
                        <a
                          href="javascript:;"
                          slot="reference"
                        >{{value.priceunit?'$':'￥'}}{{value.total_price | toFixed(value.priceunit?3:2)}}</a>
                      </el-popover>
                    </td>
                    <td>
                    ...
                    <td>
                      <div style="position:relative;">
                        ...
                        <template v-if="value.order_status==3">
                          <p
                            v-if="item.orderVo.order_status==3"
                          >{{value.order_status | filtersStatus}}</p>
                          <img
                            v-if="item.orderVo.order_status!=3"
                            src="@/assets/image/PersonalCenter/cancleMark.png"
                            alt
                            class="cancleMark"
                    ...
                    <td class="btn">
                      ...
                      <p v-if="value.confirmChangeDiliverTimeButton">
                        <el-button size="mini" @click="confirmChangeDiliverTime(value)">确认新交期</el-button>
                      </p>
                      <p v-if="value.cancleButton">
                        <el-button size="mini" @click="cancleOrder(2,value)">取消此零件</el-button>
                      </p>
                      <p v-if="value.receivingGoodsButton">
                        <el-button size="mini" @click="confirmRecieveGoods(value.id)">确认收货</el-button>
                      </p>
                      <div v-if="value.reason" class="cancleReason">取消原因：{{value.reason}}</div>
                      <p>
                        <el-button size="mini" @click="rate(value)" v-if="value.commentButton">去评价</el-button>
                      </p>
                    </td>
                  </tr>
                </template>
          ...
          <div class="fr total relative" style="position:relative;">
            <img
              v-if="item.orderVo.order_status==3"
              src="@/assets/image/PersonalCenter/bigCancleMark.png"
              alt
              class="cancleMark"
            />
            <p class>
              <strong class>人民币</strong>
              共{{countObj.renCount}}种零件，金额小计：
              <span
                class="right"
              >{{countObj.ren | toFixed(2)}}</span>
            </p>
            <p class>
              <strong class>美元</strong>
              共{{countObj.usdCount}}种零件，金额小计：
              <span
                class="right"
              >US${{countObj.usd | toFixed(3)}}</span>
            </p>
            <P>
              今日美元汇率牌价：
              <span class="right">{{item.orderVo.today_exchange}}</span>
            </P>
            <P>
              海关增值税13%：
              <span class="right">{{item.orderVo.order_bill | toFixed(2)}}</span>
            </P>
            <P>
              关税：
              <span class="right">￥{{item.orderVo.guanshui_total | toFixed(2)}}</span>
            </P>
            <P>
              清关服务费：
              <span class="right">￥{{item.orderVo.guanshui_service | toFixed(2)}}</span>
            </P>
          </div>
          <div class="footer" v-if="item.orderVo.order_status!=3">
            订单总额：
            <label>￥{{item.orderVo.order_amount | toFixed(2)}}</label>
          </div>
        </div>
      </div>
    </div>
    <SetTankuang :title="'更新交期提示'" v-if="dialogVisible" @closeDialogCallBack="dialogVisible=false">
      ...
      <div
        slot="footer"
        class="dialog-footer AcceptNewTime"
    ...
    >
      ...
      <div class="cancleOrderReason">
        <div class="reason">
          <el-input
            v-model="cancleOrderForm.reason"
            placeholder="取消订单的原因"
            @mouseenter="showcanclereasonList=true"
          ></el-input>
    ...
    >
      <div class="dialog-body" slot="dialog-body">
        <div class="RemittancNotes">
          <div
            class="color"
            style="text-align:center;margin-bottom:10px;font-weight:bolder;font-size:16px"
          >{{cancleOrderPay.desc}}</div>
          ...
          <div class="desc" v-if="cancleOrderPay.paytype==0">
            ...
            <p>
              <label for>汇款方式：</label>
              <span>1. 通过专属帐号进行线下汇款充值 > 2. 然后在此处上传汇款凭证</span>
            </p>
            <p>
              <label for>查看结果：</label>
              <span>平台审核结果会以短信或者微信公众号推送给您</span>
            </p>
            <p>
    ...
    <el-dialog title="选择付款方式" :visible.sync="selectedPayType" width="30%">
      <div>
        ...
        >温馨提示：系统检测到您是火狐浏览器，选择支付宝支付时如若失败请去支付宝官网下载安全控件</div>
      </div>
      <span slot="footer" class="dialog-footer">
...
export default {
  ...
  methods: {
    ...
    getorderDetail() {
      ...
      axios
        .request({
          ...buyerOrderCenter.queryOrderDeatil,
      ...
      if (this.cancleOrderPay.type == 0) {
        ...
        axios
          .request({
            ...
            method: "post",
            data: { orderId: this.cancleOrderPay.id },
          ...
          .then(res => {
            ...
            document
              ...
              .getElementsByTagName("form")[0].charset = "UTF-8";
            document
              .getElementById("divId")
              .getElementsByTagName("form")[0].target = "_blank";
            document
              .getElementById("divId")
