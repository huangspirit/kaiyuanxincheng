<template>
  <div class="ShoppingCartSelect">
                  ...
                  <div class="wrapInfo clear content">
                    ...
                    <div class="score">
                      ...
                      <div class="scoreitem">
                        ...
                        <span
                          class="color"
                          v-if="item.mapComment.profileA>item.mapComment.profileB"
                        >
                      ...
                      <div class="scoreitem">
                        ...
                        <span
                          class="color"
                          v-if="item.mapComment.deliverA>item.mapComment.deliverB"
                        >
                      ...
                      <div class="scoreitem">
                        ...
                        <span
                          class="color"
                          v-if="item.mapComment.serviceA>item.mapComment.serviceB"
                        >
                    ...
                    <p style="center">
                      <span
                        class="borderColor"
                        style="padding:2px 5px;border-radius:4px;cursor:pointer;font-size:14px;"
                        @click="chipShop(item)"
                      >进入店铺</span>
                    </p>
                  </div>
              ...
              <template v-for="(item1,index) in item.list">
                <ul class="listDetail" :key="index" :class="!item1.isenable?'unenable':''">
                  <li class="imgWrap" style="width:15%;">
                    ...
                    <div>
                      <ImgE :src="item1.goodsImageUrl" :W="76" :H="76"></ImgE>
                    </div>
                  </li>
                  <li style="width:20%;" class="info">
                    <p class="name" :title="item1.goods_name">
                      <router-link
                        :to="{
                      ...
                      >
                        <strong>{{item1.goods_name}}</strong>
                      </router-link>
                    </p>
                    <template>
                      <p class="desc">
                  ...
                  <li style="width:17%" class="place">
                    ...
                    <template>
                      <p
                        v-if="item1.goods_type && item1.day_interval"
                      >{{item1.day_interval | filterhours}}小时内交货</p>
                  ...
                  <template>
                    <li style="width:10%" class="price" v-if="item1.priceType">
                      <p v-for="val in item1.priceList" :key="val.price" style="display:flex; justify-content: space-around;">
                    ...
                    <div class="countTime">
                      <div>
                        <CountTime
                          ...
                          :secondsTxt="'秒'"
                        ></CountTime>
                        <p style="text-align: center;" v-if="!item1.isenable">商品已下架</p>
                      </div>
                    </div>
                  </li>
                </ul>
    ...
    <el-dialog title="选择报关方式" :visible.sync="showDeclareType" width="500">
      <div style="margin-top:-20px;" >
        <h3>
          您有
          <strong>{{declareTypeCount}}</strong>种零件需要从香港发货，请选择报关的方式
        </h3>
        <div class="hebing">
          <div :class="declareType?'borderColor':''" @click="declareType=true">
            <el-button :class="declareType?'bgColor':'bgLightGray'" >合并报关</el-button>
            <span>(合并报关是指所有香港交货零件收齐后一起报关，并只收取一次的清关服务费用)
              </span>
          </div>
          <div  :class="declareType?'':'borderColor'" @click="declareType=false">
            <el-button :class="declareType?'bgLightGray':'bgColor'">分开报关</el-button>
            <span>(单独报关是每一件零件在收货后单独报关，并每次单独收取一次清关服务费用 )</span>
          </div>
        </div>
        <div style="margin-top:15px;">
          <a  @click="showService=true">清关服务费收费标准</a>
        </div>
      </div>
    ...
    <el-dialog title="清关服务费收取标准" :visible.sync="showService" width="500" >
      <div class="showService">
        ...
        <div class="top bottom">
          <p>举例：1、A 零件，1000只， 单价：USD5.0  当天汇率：7.0 
            </p>
            <p>
              <span style="width:38px;"></span>
      2、B 零件,  2000只， 单价：USD3.0  当天汇率：7.0</p>
        </div>
        <div class="bottom">
          <p>清关服务费：</p>
          <p>合并报关=A（1000*5.0*7.0）*0.5%+B(2000*3.0*7.0)*0.5%= 385元；</p>
          <p>单独报关=A （300）+B（300）=600元；</p>
          <p class="color">提示：单次报关总货值为人民币6万元时，报关服务费为300元，低于300元按300元收取；</p>
        </div>
      </div>
    </el-dialog>
  </div>
...
export default {
  ...
  methods: {
    ...
    init() {
      ...
      axios.request({ ...shoppingCar.inquiryList, params: obj }).then(res => {
        this.goodsList = res.data.data.map(item => {
          item.list.map(item0 => {
            if (item0.goodsCount) {
              item0.count = item0.goodsCount;
            } else {
              item0.count = item0.moq;
            }
            if (item0.priceLevel) {
              item0.priceList = ladderPrice(item0.priceLevel);
              if(item0.count<item0.moq){
                  item0.money = item0.count * parseFloat( item0.priceList[0].price);
              }else{
                for(var i=item0.priceList.length-1;i>=0;i--){
                if(item0.count>=Number(item0.priceList[i].num)){
                  item0.money = item0.count * parseFloat( item0.priceList[i].price);
                 return;
                }
              }
              }
            } else {
              item0.money = item0.count * item0.goodsPrice;
            }
            if (!item0.isenable) {
              this.disabledNum++;
            }
    ...
    handleBlur(event, k, index) {
      ...
      this.handleChange(obj.count, k, index);
    },
