<template>
  <div class="BasicInforma allWidth">
    ...
    <div class="BasicInforma-con">
      ...
      <div>
        ...
        >
          ...
          <el-checkbox v-model="hasReading" style="margin-right:2px;" disabled></el-checkbox>请先阅读并接受《
          <strong @click="showXieyi=true"><a href="javascript:;">{{title}}供应商申请入驻协议</a></strong>》
        </div> 
            <div class="BasicInforma-footer" style="margin-top:10px;">
    ...
    <el-dialog :visible.sync="showXieyi" width="60%" style="height:95%;" class="xieyiwrap" top="6vh" >
      <div slot="title">
         <h3 >大麦晶城供应商申请入驻协议</h3>
      </div>
        <div class="xieyi" >
        
           <div class="content"> <p class="desc">欢迎来到大麦晶城ICmap！</p>
          ...
          <p>此协议是您与北京晶圆电子有限公司（www.icmap.cn）共同签订，自您阅读并同意本协议之时即刻生效，本协议具有合同效力，协议双方等同于合同双方。</p>
            ...
            <div style="text-align:center;" class="blu">
              <br />
              <el-checkbox v-model="hasReading"  class="color">
                我已阅读并接受
              </el-checkbox>
               <el-checkbox v-model="hasnoReading"  class="gray">
                我再想想
              </el-checkbox>
            </div>
          </div>
...
export default {
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          if(this.ruleForm.residencetype ==3 ){
            ...
            }else if(this.ruleForm.EndselectBrandList && this.ruleForm.EndselectBrandList.length){
                this.setJoinForm({...this.ruleForm});
            }else{
              this.$message({
                message:"请选择品牌"
          ...
          if(!this.hasReading){
            this.$message({
              message:`请先阅读并接受${this.title}供应商申请入驻协议`
            });
            return;
          }
  ...
  mounted() {
    ...
    this.GetQueryDictionarieList({
      ...
      let typeresidenceList=[];
      res.forEach((item,index)=>{
        if(item.id==1){
          item.name="原厂商";
          typeresidenceList[0]=item;
          if(this.ruleForm.residencetype){
          this.residencetype = this.ruleForm.residencetype;
        }else{
            this.ruleForm.residencetype = item.id;
            this.residencetype = item.id;
        }
        }else if(item.id==2){
          item.name="代理商";
          typeresidenceList[1]=item;
        }else if(item.id==3){
          item.name="分销商(个人)";
          typeresidenceList[3]=item;
        }else if(item.id==18){
          item.name="分销商(企业)";
          typeresidenceList[2]=item;
        }
        if(index==3){
          this.typeresidenceList = typeresidenceList;
        }
      })
      console.log( typeresidenceList)
    });
